...
def dashboard_view(request):
    ...
    if user.is_driver:
        print("Hello driver")
        return render(request,'registration/driver-dashboard.html',{'profile':profile})
    ...
    if request.method == 'POST':
        ...
        if data.get('type') == 'coordinates':
            ...
            if distance is not None:
                ...
                Offers.objects.create(distance=distance,fromCoordinatesLat = latA,fromCoordinatesLong = lonA)
        ...
        if data.get('type') == 'get-offers':
            ...
            try:
                offers = Offers.objects.filter(
                    ...
                    lat_diff__range=(lat_diff - (0.5 * distance_threshold_km / earth_radius_km), lat_diff + (0.5 * distance_threshold_km / earth_radius_km)),
                    lon_diff__range=(lon_diff - (0.5 * distance_threshold_km / earth_radius_km), lon_diff + (0.5 * distance_threshold_km / earth_radius_km)),
                    is_active=True,
                    is_accepted=False
                ).annotate(
                    distance_km=(sin(lat_diff / 2) ** 2 + cos(latA_rad) * cos(F('fromCoordinatesLat')) * sin(lon_diff / 2) ** 2) ** 0.5 * 2 * earth_radius_km,
        ...
        if data.get('type') == 'fetch-rides':
            offer = Offers.objects.get( user_id = data.get('user_id'),is_active = True, is_accepted = True)
            driverid = offer.driver_id
            driver = User.objects.get(id = driverid)
            if offer is None:
